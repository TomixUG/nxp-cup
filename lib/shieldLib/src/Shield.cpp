...
{
    //input valid check
    if (pwrL < -100.0) pwrL = -100.0;    
    if (pwrL >  100.0) pwrL =  100.0;
    //input valid check
    if (pwrR < -100.0) pwrR = -100.0;    
    if (pwrR >  100.0) pwrR =  100.0;
    
         if (pwrL < 0) { MA1 = pwrL / -100.0;   MA2 = 0; }
    else if (pwrL > 0) { MA1 = 0;               MA2 = pwrL / 100.0; }
    else { MA1 = 0; MA2 = 0; }
    
         if (pwrR < 0) { MB1 = 0;               MB2 = pwrR / -100.0; }
    else if (pwrR > 0) { MB1 = pwrR / 100.0;    MB2 = 0; }
    else { MB1 = 0; MB2 = 0; }
}
